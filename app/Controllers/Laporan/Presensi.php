...
class Presensi extends BaseController
{
    ...
    public function presensitenagakerja()
    {
        ...
        if ($aksi == 'detail') {
            ...
            $this->dtContent['page'] = "laporan_presensi";
            ...
            $this->dtContent['page'] = "laporan_presensi_rekap";
        }

        if ($aksi == "cetak_presensi") {
        } else if ($aksi == "export_presensi_to_xls") {
            $nm_mitrakerja = $selDtMitrakerja['singkatan'];

            $dtPresensi = $this->dbHelper->getPresensiTenagakerjaByMitraKerja($this->appID, $selPeriode, $selDtMitrakerja);

            $dtRekapPresensi = $this->dbHelper->getRekapPresensiTenagakerjaByMitraKerja($this->appID, $selPeriode, $selDtMitrakerja);
            $this->export_presensi_to_xls($selPeriode, $dtPresensi, $dtRekapPresensi, $jmlHariKerja, $nm_mitrakerja);
            exit();
        }
    ...
    public function presensitkdetail()
    {
        $actionkey = "detail";
        ...
        $appJSFoot = loadJS("presensi/laporan_presensi_tk_detail.js", "appjs");
        ...
        $this->dtContent['page'] = "laporan_presensi_tk_detail";
    ...
    public function export_presensi_to_xls($periode, $dtPresensi, $dtRekapPresensi, $jmlHariKerja, $nmMitrakerja = "LAPORAN")
    {
        $periodeBln = strtoupper(ambil_bulan_tahun($periode));

        $inputFileName = 'templates/presensi/presensi.xlsx';
        $outputFileName = $nmMitrakerja . ' - PRESENSI - ' . $periodeBln . '.xls';
